...
-- Обход ограничений ALTER для повторного запуска через DO-блок
DO $$
BEGIN
    -- Переименовываем колонку, только если старая еще существует
    ...
    -- Добавляем констреинт, только если его еще нет
...
TRUNCATE developers, games, users, purchases, reviews RESTART IDENTITY CASCADE;
...
-- Проверка существования ролей перед созданием
DO $$
BEGIN
